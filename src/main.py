...
{% for m in messages %}
<article class="msg" id="m-{{ m.safe_id }}" data-uid="{{ m.safe_id }}" data-priority="{{ m._priority }}" data-plain='{{ m.plain_b64|safe }}' data-plain-len="{{ (m.plain_b64|default(''))|length }}">
  <div class="head">
    <div>
      <div class="title-row">
...
(function(){
  ...
  // Robust renderer with fallback to messages/<uid>.json
